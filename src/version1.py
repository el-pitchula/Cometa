...
class ImageProcessingApp:
    def __init__(self, root):
        ...
        # Opção de Tipo de Filtro
        ttk.Label(control_panel, text="Filter Type").pack(pady=5)
        self.filter_type = tk.StringVar(value="low")
        ttk.Radiobutton(control_panel, text="Low-pass", variable=self.filter_type, value="low").pack(anchor="w")
        ttk.Radiobutton(control_panel, text="High-pass", variable=self.filter_type, value="high").pack(anchor="w")
    ...
    def apply_butterworth_filter(self):
        ...
        self.processed_image = self.butterworth_filter(self.original_image, cutoff=self.cutoff_slider.get(), filter_type=filter_type)
        self.display_image(self.processed_image, self.processed_canvas)
        self.status_label.config(text=f"Butterworth {filter_type} filter applied")

    def butterworth_filter(self, image, cutoff, filter_type='low'):
        ...
        for i in range(rows):
            for j in range(cols):
                ...
                if filter_type == 'low':
                    mask[i, j] = 1 / (1 + (d / cutoff) ** (2 * 1)) # n=1 (1° ordem)
                elif filter_type == 'high':
                    mask[i, j] = 1 / (1 + (cutoff / d) ** (2 * 1))
